...
def _log_loss_config():
    ...
    if LOSS_CONFIG.tda.enabled:
        tda_terms = [
            name
            for name, term in LOSS_CONFIG.tda.terms.items()
            if term.enabled
        ]
        if tda_terms:
...
def _train_point_cloud(name, target_adj, target_pts, optimizer, scheduler, *, get_pred_pts, loss_fn, h0rff, h1rff, h2rff, visualize=False):
    ...
    target_diags = pd_from_graph(target_adj, **LOSS_CONFIG.tda.pd)
    ...
    hom_label = "/".join(f"H{d}" for d in range(LOSS_CONFIG.tda.pd.hom_dim))
    ...
    for step in range(MLP_STEPS + 1):
        ...
        pred_diags = pd_from_graph(pred_adj, **LOSS_CONFIG.tda.pd)
        w = 1
        ...
        if step % LOG_EVERY == 0 or step == MLP_STEPS:
            ...
            pred_counts = [len(pred_diags[i]) for i in range(LOSS_CONFIG.tda.pd.hom_dim)]
