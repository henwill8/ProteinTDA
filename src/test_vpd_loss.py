...
def main() -> None:
    ...
    h0rff = create_heat_random_fourier_features(**HEAT_RFF_CONFIG["h0rff"], show_progress=True)
    h1rff = create_heat_random_fourier_features(**HEAT_RFF_CONFIG["h1rff"], show_progress=True)
    print(f"Time taken to create heat kernels: {time.time() - timer:.2f} seconds")

    print("Running cases...")
    pd_a = torch.tensor([[0.0, 0.5], [0, 1.5]], dtype=torch.float64)
