...
def main() -> None:
    ...
    pd_a = torch.tensor([[0.0, 1.25]], dtype=torch.float64)
    pd_b = torch.tensor([[0.0, 1.0], [1.0, 1.25]], dtype=torch.float64)
    pd_same = torch.tensor([[0.0, 0.0], [1.0, 1.0]], dtype=torch.float64)
    ...
    print(f"vpd_h0: {h0rff.get_vpd(pd_a)}")
    print(f"vpd_h1: {h1rff.get_vpd(pd_b)}")
